...
if __name__ == '__main__': 
    ...
    if SlaveAddrNum <= 0:
        ...
        print("")
    # 向0x00地址开始写3字节数据
    WriteBuffer = (c_byte * 3)()
    WriteBuffer[0] = 0x00
    WriteBuffer[1] = 0x41
    WriteBuffer[2] = 0x27
    # for i in range(0,8):
    #     WriteBuffer[1+i] = i
    ret = IIC_WriteBytes(DevHandles[DevIndex],IICIndex,0x43,byref(WriteBuffer),3,100)
    if ret != IIC_SUCCESS:
    ...
    # for i in range(0,8):
    #     WriteBuffer[1+i] = i
    ret = IIC_WriteReadBytes(DevHandles[DevIndex],IICIndex,0x43,byref(WriteBuffer),1,byref(ReadBuffer),2,100)
    if ret != IIC_SUCCESS:
        print("WriteRead iic faild!")
        exit()
    else:
        print("WriteRead iic sunccess!")
        print("Read 0x01 Data:")
        for i in range(0,len(ReadBuffer)):
            print("%02X "%ReadBuffer[i],end='')
        print("")
    
    # 从0x02地址开始读取8字节数据
    ReadBuffer = (c_uint16 * 2)()
    WriteBuffer[0] = 0x02
    # for i in range(0,8):
    #     WriteBuffer[1+i] = i
    ret = IIC_WriteReadBytes(DevHandles[DevIndex],IICIndex,0x43,byref(WriteBuffer),1,byref(ReadBuffer),8,100)
    if ret != IIC_SUCCESS:
        print("WriteRead iic faild!")
        exit()
    else:
        print("WriteRead iic sunccess!")
        print("Read 0x02 Data:")
        for i in range(0,len(ReadBuffer)):
            print("%02X "%ReadBuffer[i],end='')
        print("")
